...
export const metaverseWorldPlacedWaterRegions = Object.freeze(
  metaverseWorldWaterRegions.map((waterRegion) =>
    createPlacedWaterRegionSnapshot(waterRegion)
  )
) as readonly MetaverseWorldPlacedWaterRegionSnapshot[];
...
export function resolveMetaverseWorldDynamicSurfaceColliders(
  ...
  if (
    ...
    dynamicSurfaceAsset.placements.length === 0
  ) {
    return emptyMetaverseWorldPlacedSurfaceColliders;
  }

  const authoredPlacement = dynamicSurfaceAsset.placements[0]!;
  ...
  return Object.freeze(
    dynamicSurfaceAsset.surfaceColliders.map((collider) =>
      createPlacedSurfaceColliderSnapshot(environmentAssetId, collider, placement)
    )
  );
}
