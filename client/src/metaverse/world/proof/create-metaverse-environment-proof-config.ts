...
import type { LoadedMetaverseMapBundleSnapshot } from "../map-bundles";
...
function resolveSurfaceMeshMaterialId(
  region: Pick<
    import("@webgpu-metaverse/shared").MetaverseMapBundleSemanticRegionSnapshot,
    "materialReferenceId" | "regionKind"
  >
): import("@webgpu-metaverse/shared").MetaverseMapBundleSemanticMaterialId {
  switch (region.materialReferenceId) {
    case "alien-rock":
    case "concrete":
    case "glass":
    case "metal":
    ...
    case "team-red":
    case "warning":
      return region.materialReferenceId;
  }
...
export function createMetaverseEnvironmentProofConfig(
  ...
  const surfacesById = new Map(
    loadedBundle.bundle.semanticWorld.surfaces.map((surface) => [
      surface.surfaceId,
      surface
    ] as const)
  );
  ...
  return Object.freeze({
    ...
    gameplayVolumes: Object.freeze(
      loadedBundle.bundle.semanticWorld.gameplayVolumes.map((volume) =>
        Object.freeze({
    ...
    lights: Object.freeze(
      loadedBundle.bundle.semanticWorld.lights.map((light) =>
        Object.freeze({
          color: light.color,
    ...
    materialDefinitions: loadedBundle.bundle.semanticWorld.materialDefinitions,
    proceduralStructures: Object.freeze(
      loadedBundle.bundle.semanticWorld.structures.map((structure) =>
        Object.freeze({
    ...
    surfaceColliders: resolveMetaverseMapBundleCompiledWorldSurfaceColliders(
      loadedBundle.bundle.compiledWorld
    ),
    surfaceMeshes: Object.freeze(
      loadedBundle.bundle.semanticWorld.regions.flatMap((region) => {
        if (region.regionKind === "arena") {
          return [];
        }
        ...
        const mesh = createMetaverseMapBundleSemanticRegionSurfaceMesh(
          region,
          surface
        );
        ...
        return mesh === null
          ...
          : [
              Object.freeze({
                ...
                materialId: resolveSurfaceMeshMaterialId(region),
    ...
    terrainPatches: Object.freeze(
      loadedBundle.bundle.semanticWorld.terrainPatches.map((terrainPatch) =>
        Object.freeze({
          heightSamples: terrainPatch.heightSamples,
          materialLayers: Object.freeze(
            terrainPatch.materialLayers.map((materialLayer) =>
              Object.freeze({
                layerId: materialLayer.layerId,
                materialId: materialLayer.materialId,
                weightSamples: materialLayer.weightSamples
              })
            )
          ),
